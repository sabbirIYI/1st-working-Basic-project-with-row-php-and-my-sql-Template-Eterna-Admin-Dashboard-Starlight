<?php
    ...
    if (!$job_position) {
     ...
     if ($not_error) { 
        ...
        $Update_query = " UPDATE `testimonial` SET `client_name` = '$client_name', `job_Position` = '$job_position', `testimonial_details` = '$testimonial_details', `img_location` = '$img_folder_location' WHERE `id` = $id";

        mysqli_query(connect_database(),  $Update_query);

        $_SESSION ['testimonial_updated'] = "Testimonial Updated Successfully";
