<?php
    ...
    if (!$job_position) {
     ...
     if ($not_error) { 
        ...
        $Update_query = " UPDATE `team` SET `member_name` = '$member_name', `job_position` = '$job_position', `short_details` = '$short_details', `img_location` = '$img_folder_location' WHERE `id` = $id";

        mysqli_query(connect_database(),  $Update_query);

        $_SESSION ['team_updated'] = "Team Updated Successfully";
